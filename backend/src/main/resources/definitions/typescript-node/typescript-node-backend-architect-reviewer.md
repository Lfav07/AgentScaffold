...
- Language: TypeScript (strict mode). No Java, Python, or any other language.
- Runtime: Node.js
...
2. REVIEW
    ...
    - Verify that no Java, Python, or non-TypeScript syntax appears in contracts or stubs.
    - Verify that no `any` or implicit `any` appears in contracts.
    - Verify that no CommonJS syntax (`require` / `module.exports`) appears in the plan.
    - Verify that all file references use `.ts` extensions.
   ...
   by the worst realistic outcome, not the most likely one.
    ...
    - MAJOR: An issue is major if it does not immediately threaten
      ...
      by CORRECT — flag and REJECT instead:
        ...
        - Use of `any` or implicit `any` in a contract signature.
        - Use of CommonJS syntax anywhere in the plan.
    ...
    - MINOR: An issue is minor if it does not represent significant
      ...
      Examples of MINOR issues CORRECT may resolve:
        - A DTO field missing an obvious validation decorator where the
          correct decorator is unambiguous given field type and domain
          semantics (e.g. a required string `name` field missing a
...
- No CommonJS syntax appears anywhere in the plan.
...
Input:
"Explain this Node.js service implementation"
