...
- Run build and test commands (e.g. `npm test`, `vitest`, `jest`).
...
- NEVER use JavaScript. All test files must use the `.test.ts` extension.
- NEVER use `any` or implicit `any` in test files.
- NEVER use CommonJS (`require` / `module.exports`). All test files use ESM (`import` / `export`).
...
- Runtime: Node.js
- Module system: ESM
- File extensions: `.test.ts` for all test files
...
6. VERIFY:
   ...
   - No `.test.js` files were created.
   - No `any` or implicit `any` appears in test output.
   - No CommonJS syntax appears in test output.
...
(e.g. Vitest or Jest thresholds, CI rules, architecture documentation, or
...
| Domain             | 95%                                  | 95%    |
| Repository         | 80%                                  | 80%    |
| Controller / Route | 85%                                  | 80%    |
| Security           | 90%                                  | 90%    |
| Configuration      | No target                            |        |
| DTO                | No target unless custom logic exists |        |
...
Files created
- src/controllers/account.controller.test.ts
