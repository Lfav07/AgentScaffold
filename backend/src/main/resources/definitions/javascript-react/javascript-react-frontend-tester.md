...
- NEVER use TypeScript. All test files must use `.test.js` or `.test.jsx` extensions.
- NEVER use TypeScript syntax in test files: no type annotations, no interfaces, no generics, no `as` casts.
...
- No `.test.ts` or `.test.tsx` files were created.
- No TypeScript syntax appears anywhere in the test output.
