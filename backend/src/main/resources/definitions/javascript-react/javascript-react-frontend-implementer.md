...
- NEVER use TypeScript. All files must use `.js` or `.jsx` extensions.
- NEVER use TypeScript syntax: no type annotations, no interfaces, no `as` casts, no generics, no `?.` type guards
  introduced solely for type safety.
- Define prop contracts using PropTypes declarations at the bottom of each component file.
- Document non-trivial functions and hooks with JSDoc `@param` / `@returns` annotations.
- NEVER use class components. Use functional components and hooks only.
...
- The project builds cleanly (no compile or lint errors)
...
- No `.ts` or `.tsx` files were created
- All components declare PropTypes
- No TypeScript syntax appears anywhere in the output
...
Implemented the Customer list and detail views according to the approved implementation plan. Data fetching is handled via `useCustomers` hook using TanStack Query, keeping components purely presentational. PropTypes declared on all components. Existing layout and UI primitives were reused. No assumptions or blockers.
...
Implemented the Dashboard layout and RevenueChart component shell. PropTypes are declared on all components. The chart rendering could not be completed because the `recharts` package was unavailable in the project environment. The component accepts the expected props and is wired up, but renders a placeholder until the dependency is installed.
...
Reason:
The implementation plan does not specify the state management library or pattern 
(e.g., Zustand, Redux, React Context). Resolving this would require an architectural decision outside the Implementer's responsibility.