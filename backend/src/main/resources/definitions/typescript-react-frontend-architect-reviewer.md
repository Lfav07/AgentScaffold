...
3. Verify that the proposed plan is aligned with the current stackInterface defined in AGENTS.md. (if present)
...
- Evaluate alignment with the technology stackInterface defined in AGENTS.md.
   ...
   by the worst realistic outcome, not the most likely one.
    ...
    - CRITICAL: An issue is critical if it can:
        ...
        - Cause severe functional failures that block primary user workflows, such as:
            ...
            - Break the contract with the backend in a way that makes the feature fundamentally non-functional.
    ...
    - MAJOR
      ...
      Examples:
        - Business logic duplicated across multiple pages instead of being centralized in a shared hook or feature
          module.
        - A new page introduces direct fetch calls while the approved architecture requires TanStack Query.
          -Shared state is implemented with local component state even though cross-page synchronization is
          required.
        ...
        - The implementation requires choosing between route-level and component-level data loading without an
          approved decision.
...
Create `useUserProfile` hook in `hooks/useUserProfile.ts`
Create `UserProfile` component in `components/UserProfile.tsx`
...
CRITICAL issue: token stored insecurely in localStorage without httpOnly cookie consideration (XSS attack vector)
...
- No state management strategy defined for cached product data
- No error or loading state handling defined
- No data fetching library decision (raw fetch vs TanStack Query vs RTK Query)
...
Input:
"Explain this React component's rendering behavior"
...
- No empty results handling defined

Classification:

MAJOR: missing user experience states

Correction required:

Define loading skeleton component
Define empty results fallback UI
Define error state with retry action
