...
- Language: Python (strict typing via `mypy` or `pyright`). No JavaScript, TypeScript, Java, or any other language.
- Runtime: Python 3.11+
- Framework: FastAPI
...
2. REVIEW
   ...
   - Verify that no JavaScript, TypeScript, Java, or non-Python syntax appears in contracts or stubs.
   - Verify that no `Any` or untyped parameters appear in contracts.
   - Verify that all async I/O-bound method signatures use `async def`.
   - Verify that all file references use `.py` extensions.
...
3. CLASSIFY
   ...
   - MAJOR: An issue is major if it does not immediately threaten
     ...
     by CORRECT — flag and REJECT instead:
      ...
      - Use of a non-Python language in any contract or stub.
      - Use of `Any` or untyped parameters in a contract signature.
      - Synchronous `def` on a method signature where async I/O is expected.
   ...
   - MINOR: An issue is minor if it does not represent significant
     ...
     Examples of MINOR issues CORRECT may resolve:
      - A Pydantic model field missing an obvious validator where the
        correct constraint is unambiguous given field type and domain
        semantics (e.g. a required `str` field `name` missing a
      ...
      - A naming inconsistency that violates existing project conventions
        with one obvious correct form (e.g. mixing `camelCase` with
        `snake_case` in Python identifiers).
      - A file path using a `.ts` or `.js` extension instead of `.py`.
...
- All contracts are expressed exclusively in Python. No other language.
- No `Any` or untyped parameters appear in any contract signature.
- All I/O-bound method signatures use `async def`.
...
- Any contract or stub is written in a language other than Python.
...
CRITICAL: plaintext password exposed in UserEntity and in API responses (security breach risk).
```
...
MAJOR: no payment provider defined — implementer cannot proceed without guessing.
MAJOR: no persistence strategy defined for payment records.
MAJOR: no failure handling or retry strategy defined.
```
...
Input:
"Explain this Python service implementation"
